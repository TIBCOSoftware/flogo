...
ext_docs() {
    ...
    for i in `find katacoda-scenarios -name \*interactive-lab.md` ; do filename=$(basename $i); title=`echo | awk 'NR==2 {print;exit}' $i`; link=`echo | awk 'NR==3 {print;exit}' $i`; echo "{{< bigcard img=\"../images/labs/047-safety-glasses.svg\" headercolor=\"bg3\" text=\"${title:7}\" href=\"${link:5}\">}}" >> docs/content/labs/_index.md ; done;
